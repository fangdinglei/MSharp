...
using MSharp.Core.Compile.Method;
...
namespace MSharp.Core.Compile
{
    static internal class ClassAnalyzer
    {
        ...
        /// <returns>cpu main method</returns>
        static public LMethod? AnalyzeCPU(CompileContext context, INamedTypeSymbol classSymbol, LClass @class)
        {
            ...
            foreach ((var methodSymbol, var method) in @class.Functions)
            {
                ...
                {
                    MethodAnalyzer.AnalyzeBody(context, methodSymbol, method, true, true);
                    return method;
                }
            }
            return null;
